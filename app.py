...
.kpi-card { background: linear-gradient(135deg, #13213a 0%, #1a2d4a 100%); border: 1px solid #1e3a5f; border-radius: 12px; padding: 20px 24px; text-align: center; min-height: 145px; display: flex; flex-direction: column; justify-content: center; position: relative; overflow: hidden; transition: transform 0.2s; }
...
.kpi-sub { color: #4a7a9b; font-size: 0.7rem; margin-top: 6px; }
...
.page-title { font-family: 'Rajdhani', sans-serif; font-size: 2.6rem; font-weight: 700; color: #e8d5a3; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 0; line-height: 1; }
...
with st.sidebar:
    ...
    price_range = st.slider("💰 Price Range (AED)", price_min, price_max, (price_min, min(price_max, 500_000)))
...
if selected_city != "All": df = df[df["Location"] == selected_city]
if selected_fuel != "All": df = df[df["Fuel Type"] == selected_fuel]
if selected_trans != "All": df = df[df["Transmission"] == selected_trans]
if selected_make != "All": df = df[df["Make"] == selected_make]
...
kpis = [
    ...
    (k6, "🏎️", str(df["Make"].value_counts().index[0] if not df.empty else "-"), "Top Brand")
]
for col, icon, val, label in kpis:
    with col:
        st.markdown(f'<div class="kpi-card"><div class="kpi-icon">{icon}</div><div class="kpi-label">{label}</div><div class="kpi-value">{val}</div></div>', unsafe_allow_html=True)
...
with col3:
    ...
    fig.update_layout(title="Cars Listed by Year")
    st.plotly_chart(apply_theme(fig), use_container_width=True)
with col4:
    if "Make" in df.columns:
        make_cnt = df["Make"].value_counts().head(10).reset_index()
        make_cnt.columns = ["Make", "Count"]
        fig = px.bar(make_cnt, x="Make", y="Count", title="Top 10 Car Brands")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
...
with col5:
    if "Make" in df.columns:
        ...
        fig = px.bar(top_price, x="Make", y="Price", title="Top 10 Makes by Price")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
with col6:
...
with col7:
    if "Model" in df.columns:
        ...
        fig = px.bar(model_p, x="Price", y="Model", orientation="h", title="Top Models by Price")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
with col8:
    if "Transmission" in df.columns:
        fig = px.pie(df, names="Transmission", hole=0.5, title="Transmission Type")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
with col9:
    if "Color" in df.columns:
        color_c = df["Color"].value_counts().head(9).reset_index()
        fig = px.line(color_c, x="Color", y="count", title="Top Colors")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
...
with col10:
    ...
    fig.update_layout(title="Avg Price by Year")
    st.plotly_chart(apply_theme(fig), use_container_width=True)
with col11:
    if "Fuel Type" in df.columns:
        fig = px.box(df[df["Price"] < df["Price"].quantile(0.97)], x="Fuel Type", y="Price", color="Fuel Type", title="Price Range by Fuel Type")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
...
with col12:
    ...
    fig.update_layout(title="Car Count Forecast (ML)")
    st.plotly_chart(apply_theme(fig), use_container_width=True)
with col13:
    ...
    fig.update_layout(title="Total Market Value Forecast")
    st.plotly_chart(apply_theme(fig), use_container_width=True)
